<div x-data="{ secondsLeft: {{ $currentQuestion->timer }} }" x-init="setInterval(() => {
    if (secondsLeft > 1) { secondsLeft--; } else {
        secondsLeft = {{ $currentQuestion->timer }};
        $wire.nextQuestion();
    }
}, 1000);">
    ...
    @if ($currentQuestionIndex < $this->questionsCount - 1)
        <div class="mt-4">
            <x-secondary-button
                x-on:click="secondsLeft = {{ $currentQuestion->timer }}; $wire.nextQuestion();">
                Next question
            </x-secondary-button>
        </div>
        @else
